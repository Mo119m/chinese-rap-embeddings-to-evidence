...
- **Present but hidden** (`identity_probe.json`): whitening the BGE-M3 song vectors by within-author covariance, fitted on four folds of leakage groups and applied to the fifth, lifts the semantic system from **0.318 to 0.449**, level with the raw lexical one; fused with it, they reach **0.544**. Whitening by the total covariance with no labels already gives 0.432 and permuted labels give 0.431, so almost all of the recovery is the embedding's anisotropy and only +0.017 (interval +0.014 to +0.020) is author structure. The same correction lifts a 1,024-dimensional lexical space from 0.393 to **0.518**, so the surface keeps its lead under equal treatment, and the two corrected spaces fused reach **0.587**, the strongest system in the study; on 34 held-out labels the label-free whitening transfers in full (+0.156) while the label-fitted one falls short of it (−0.027).
- **Label aliases** (`results/label-identity-v1/alias_audit.json`): an author-supplied artist list (partial, 229 entries, name column only) matches 183 of 241 corpus labels and maps no entry to two labels; the region column is deliberately not read.
- **What the surface is** (`lexical_identity_anatomy.json`, `word_identity_anatomy.json`, `representation_unit.json`, `word_space_probe.json`): jieba words identify a label better than any character n-gram space (0.519 against 0.450), single words suffice (0.533) and word bigrams add nothing; the commonest quarter of the vocabulary alone reaches 0.515 and carries 96% of the correct label's score while the rarest quarter can be removed without loss; content words and English tokens matter most, function words can go, names are worth 0.014, and stripping the 605-surface catalogue before segmentation leaves 0.511. In the character space the signal sits in short Han n-grams with a less common character, Latin n-grams add nothing, and n-grams across line breaks are noise. Whitening the word space and fusing it with the chunk-whitened semantic space gives **0.608**, the strongest system tried.
