...
</style></head><body><main><div class="eyebrow">CHINESE RAP RESEARCH RELEASE · V4</div><h1>One corpus. One question. Three spaces.</h1><p class="lead">Where in the text does a source-credit label's identity live? Read the paper for the answer on corpus v2 — semantic, lexical and rhyme-form spaces under one protocol, and a whitening probe of what the embedding keeps. The interactive companion below is built on the earlier snapshot and is an interface, not evidence.</p><section class="grid">
...
<a class="card" href="Paper/Chinese_Rap_Evidence_Grounded_Manuscript.pdf"><div><span class="num">02 · READ THE STUDY</span><h2>Read the paper</h2><p>The English journal manuscript asks one question of three representation spaces under one leave-group-out protocol, decomposes the lexical advantage, and probes what the frozen BGE-M3 embedding keeps of a label's identity.</p></div><div class="open">Open the readable PDF →</div></a>
...
- **Clear research theme:** one question — where in the text a source-credit label's identity lives — asked of three representation spaces (semantic, lexical, rhyme form) under one leave-group-out protocol on corpus v2, with the cultural-reference layer serving as the neutralisation control.
- **Identity experiments with controls:** the lexical advantage is decomposed by script class and four named-reference neutralisation arms; the semantic space is probed by within-author whitening against total-covariance, permuted-label, centring, matched-dimension lexical, and held-out-author controls.
...
- **Meaningful downstream evaluation:** retrieval uses leave-group-out profiles over exact-text or near-duplicate leakage groups and paired two-stage bootstrap uncertainty; cultural-reference links use shared-text exclusion, support, conservative intervals, and BH-FDR; written-ending prediction uses leakage-group partitions, baselines, ablation, calibration, and switch diagnostics.
