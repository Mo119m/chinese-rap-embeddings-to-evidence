...
def build(private_root: Path, lexicon: Path, out_dir: Path) -> int:
    ...
    table = out_dir / "repaired_lyric_chunks_v2_neutralised.csv"
    table.write_text(buffer.getvalue(), encoding="utf-8", newline="")
    neutral_digest = corpus_content_sha256(rows)
    contract = {
        "derived_from_corpus_content_sha256": CORPUS_CONTENT_SHA256,
...
def main() -> int:
    ...
    return build(root, lexicon, out_dir)
