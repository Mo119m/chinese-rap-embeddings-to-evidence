...
CORE_REQUIRED_PATHS = {
    ...
    "methods/NER_CR_001_COMPOUND_RESOLUTION.md",
    "analysis/compound-resolution/resolution_table.csv",
    "analysis/compound-resolution/freeze.json",
    "analysis/compound-resolution/public_name_allowlist.json",
    "tests/test_compound_resolution_gate.py",
    "tests/test_release_boundary.py",
    "tools/build_compound_resolution_table.py",
    "tools/publish_compound_resolution.py",
    "tools/verify_compound_resolution.py",
    "tools/rekey_blinded_ballots.py",
...
# and the self-generated manifest regenerated around it.
REPOSITORY_TO_PACKAGE = {
    "methods/NER_CR_001_COMPOUND_RESOLUTION.md": "Methods/NER_CR_001_COMPOUND_RESOLUTION.md",
    "methods/NER_RELEASED_CLAIM_AUDIT_PROTOCOL.md": "Methods/NER_RELEASED_CLAIM_AUDIT_PROTOCOL.md",
    "analysis/compound-resolution/resolution_table.csv":
        "Results/compound-resolution-ner-cr-001/resolution_table.csv",
    "analysis/compound-resolution/freeze.json":
        "Results/compound-resolution-ner-cr-001/freeze.json",
    "analysis/compound-resolution/public_name_allowlist.json":
        "Results/compound-resolution-ner-cr-001/public_name_allowlist.json",
    "tools/verify_compound_resolution.py": "Reproducibility/tools/verify_compound_resolution.py",
    "tools/publish_compound_resolution.py": "Reproducibility/tools/publish_compound_resolution.py",
    "tools/build_compound_resolution_table.py": "Reproducibility/tools/build_compound_resolution_table.py",
    "src/build_chinese_rap_release_v4.py": "Reproducibility/src/build_chinese_rap_release_v4.py",
    "src/validate_public_release_integrity_v1.py":
        "Reproducibility/src/validate_public_release_integrity_v1.py",
    "tests/test_compound_resolution_gate.py": "Reproducibility/tests/test_compound_resolution_gate.py",
    "tests/test_tools.py": "Reproducibility/tests/test_tools.py",
}
...
def verify_desktop_package(target: Path) -> int:
    ...
    required_paths = {
        ...
        "Results/compound-resolution-ner-cr-001/freeze.json",
        "Results/compound-resolution-ner-cr-001/public_name_allowlist.json",
        "Reproducibility/tools/build_compound_resolution_table.py",
        "Reproducibility/tools/publish_compound_resolution.py",
    ...
    # the frozen resolution table is a commitment; a desktop copy that drifted from the
    # repository would put two different frozen tables under one protocol name
    for name in ("resolution_table.csv", "freeze.json"):
        packaged = target / "Results" / "compound-resolution-ner-cr-001" / name
        if packaged.read_bytes() != (ROOT / "analysis" / "compound-resolution" / name).read_bytes():
            raise AssertionError(f"Desktop copy of {name} differs from the frozen repository copy")
    ...
    with zipfile.ZipFile(archive) as package:
        ...
        # member bytes, not only names and CRC: a member can be replaced with different
        # content and a freshly computed, entirely valid CRC
        for name in sorted(archive_files):
            relative = name.split("/", 1)[1]
            with package.open(name) as member:
                member_digest = hashlib.sha256(member.read()).hexdigest()
            if member_digest != sha256(target / relative):
                raise AssertionError(f"Desktop ZIP member differs from the validated directory: {name}")
        checked += len(archive_files)

    # explicit repository -> package byte mapping for everything that carries a rule or a
    # verification, so a packaged copy cannot be edited and re-manifested
    for source, packaged in REPOSITORY_TO_PACKAGE.items():
        if (ROOT / source).read_bytes() != (target / packaged).read_bytes():
            raise AssertionError(f"Packaged {packaged} is not byte-identical to repository {source}")
    checked += len(REPOSITORY_TO_PACKAGE)

    # The authority is the committed contract plus git, never the package. The package's
    # own SOURCE_PROVENANCE.json is checked FOR AGREEMENT with the contract; it does not
    # get to define what should be there, because a tamperer can rewrite it.
    contract_path = ROOT / "validation" / "desktop_package_contract.json"
    pin_path = ROOT / "validation" / "desktop_zip.sha256"
    for required in (contract_path, pin_path):
        if not required.is_file():
            raise AssertionError(f"committed desktop pin is missing: {required.name}; a package "
                                 "cannot be validated against itself")
    contract = read_json(contract_path)
    expected_mapping = contract["package_to_repository"]
    expected_generated = set(contract["generated_by_builder"])
    if sha256(archive) != pin_path.read_text(encoding="utf-8").strip():
        raise AssertionError("Desktop ZIP does not match the committed checksum pin")

    provenance = read_json(target / "Validation" / "SOURCE_PROVENANCE.json")
    declared = {packaged: record["source"] for packaged, record in provenance["files"].items()}
    if declared != expected_mapping:
        missing = sorted(set(expected_mapping) - set(declared))
        extra = sorted(set(declared) - set(expected_mapping))
        remapped = sorted(k for k in set(expected_mapping) & set(declared)
                          if expected_mapping[k] != declared[k])
        raise AssertionError("Packaged provenance disagrees with the committed contract; "
                             f"missing={missing[:5]} extra={extra[:5]} remapped={remapped[:5]}")
    if set(provenance["generated_by_builder"]) != expected_generated:
        raise AssertionError("Packaged generated-file set disagrees with the committed contract")
    # actual_paths deliberately omits the package manifest, which describes the rest
    expected_contents = (set(expected_mapping) | expected_generated) - {
        manifest_path.relative_to(target).as_posix()}
    if actual_paths != expected_contents:
        missing = sorted(expected_contents - actual_paths)
        extra = sorted(actual_paths - expected_contents)
        raise AssertionError("Package contents disagree with the committed contract; "
                             f"missing={missing[:5]} extra={extra[:5]}")
    head = {}
    listing = subprocess.run(["git", "-C", str(ROOT), "ls-tree", "-r", "-z", "HEAD"],
                                           capture_output=True, check=True)
    for record in listing.stdout.decode("utf-8").split(chr(0)):
        if record:
            meta, path = record.split(chr(9), 1)
            _, kind, sha = meta.split()
            if kind == "blob":
                head[path] = sha
    # every mapped file must equal the committed blob of the repository path the CONTRACT
    # names, recomputed here rather than read from the package
    for packaged, source in sorted(expected_mapping.items()):
        blob = head.get(source)
        if blob is None:
            raise AssertionError(f"the contract maps {packaged} to {source}, absent from HEAD")
        payload = (target / packaged).read_bytes()
        header = f"blob {len(payload)}".encode("ascii") + b"\x00"
        if hashlib.sha1(header + payload).hexdigest() != blob:
            raise AssertionError(f"Packaged {packaged} does not match the committed blob of {source}")
    checked += len(expected_mapping) + 4
    return checked + len(required_paths) + 12


def main() -> None:
    if not (ROOT / "validation" / "MANIFEST.json").is_file() or not (ROOT / ".git").exists():
        raise SystemExit(
            "this validator runs from a repository checkout; inside the desktop package use "
            "Reproducibility/tools/verify_compound_resolution.py and "
            "Reproducibility/tests/test_compound_resolution_gate.py")
