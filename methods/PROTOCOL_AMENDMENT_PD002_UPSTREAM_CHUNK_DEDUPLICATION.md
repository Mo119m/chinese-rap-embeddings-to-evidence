...
What the measurement does establish is a bound, and the bound is the useful part. Every candidate reproduces the published vectors to a cosine of at least 0.99997, so not knowing which one was used is not a material threat to comparability. **This measurement is not yet a published artifact.** The table above was transcribed from a notebook run whose output was not captured into `results/`, so unlike every other quantitative claim here it carries no manifest, no input hashes and no validation, and a reader cannot check it. It is recorded as a measurement taken, not as an established result, and `results/embedding-configuration-probe-v1/` is the artifact that would settle it. The fp16 run is systematically the closest of the three, which is weak evidence that the original used fp16 -- one sample of 64, a gap of 9.5e-05 -- and is recorded as weak evidence, not as a recovered fact.
