...
**What it does to corpus v2's component structure.** Confirmed after the finding was raised: treating these chunks as shared text inflates the leakage structure and mis-weights real songs. Published, 6,025 components with 799 spanning more than one song and a largest of 27; excluding stray-title chunks, 6,152 components with 776 multi-song and a largest of 25. 81 song records carry a component weight below one (median 0.500, minimum 0.111) purely because they share a bled title line with another record. The weighting rule exists to stop repeated imports dominating an aggregate; here it penalises genuinely distinct songs for carrying an export artefact. Both structures are now published in `results/repaired-corpus-v2/analysis_summary.json` under `stray_title_sensitivity`.
